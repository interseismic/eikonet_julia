...
function locate(params, origins0, dtimes, stations, eikonet, scaler)
    ...
    X_sta[1:3,:] .=  (X_sta[1:3,:] .- scaler.min[4:6]) ./ scaler.scale
    X_src[2:4,:] .=  (X_src[2:4,:] .- scaler.min[1:3]) ./ scaler.scale
    ...
    X = cpu(X_src)
    X[2:4,:] = (X[2:4,:] .* scaler.scale) .+ scaler.min[1:3]
    X[2:4,:] .*= 1f3

    origins = DataFrame(time=DateTime[], evid=String[], latitude=Float32[], longitude=Float32[],
                        depth=Float32[], magnitude=Float32[], X=Float32[], Y=Float32[])
...
function syntheticdataset(pfile)
    ...
    println(maximum(eachcol(X)))
    println(minimum(eachcol(X)))
    T_pred = dropdims(model(X[2:end,:]), dims=1) .+ X[1,:]
    # T_pred = dropdims(model(X), dims=1) .+ X[1,:]
    ΔT_pred = Q*T_pred
    ...
    origins0 = DataFrame(time=DateTime[], evid=String[], latitude=Float32[], longitude=Float32[], depth=Float32[], mag=Float32[])
    for (i, origin) in enumerate(eachrow(origins))
        lat = origin.latitude + rand(Normal(0.0, 0.015))
        lon = origin.longitude + rand(Normal(0.0, 0.015))
...
function run(pfile)
    ...
    dtimes = CSV.read(params["dtime_file"], DataFrame,
        types=Dict(:network=>String, :station=>String, :phase=>String, :cc=>Float32, :dt=>Float32, :evid1=>String, :evid2=>String)
    )
    filter!(row -> row.cc >= params["cc_min"], dtimes)
    dtime_counts = Dict()
    for row in eachrow(origins0)
        dtime_counts[row.evid] = 0
    end
    for gdf in groupby(dtimes, :evid1)
        dtime_counts[gdf.evid1[1]] += nrow(gdf)
    end
    for gdf in groupby(dtimes, :evid2)
        dtime_counts[gdf.evid2[1]] += nrow(gdf)
    end

    # Filter events with fewer than n_min differential times
    filter!(((k,v),) -> v >= params["n_min_dtimes"], dtime_counts)
    # filter!(:evid => in(Set(keys(dtime_counts))), origins0)
    # filter!([:evid1, :evid2] => (evid1, evid2) -> evid1 in(Set(origins0.evid)) || evid2 in(Set(origins0.evid)), dtimes)
    # idx = []
    # for row in eachrow(dtimes)
    #     if (row.evid1 in Set(origins0.evid)) || (row.evid2 in Set(origins0.evid))
    #         push!(idx, rownumber(row))
    #     end
    # end
    # dtimes = dtimes[idx,:]
    ...
    filter!(:evid => in(Set(keys(dtime_counts))), origins)
